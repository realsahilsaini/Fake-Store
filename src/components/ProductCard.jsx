...
const ProductCard = ({productData}) => {
  ...
  return (
    <Link to={`/details/${productData.id}`} className='flex flex-col justify-center items-center max-h-[110%] w-[45%] md:w-[25%] md:max-h-[80%] border-2 border-neutral-700 shadow rounded-md overflow-hidden'>
      ...
      <div className='py-4 flex justify-center items-center max-h-[85%]: md:max-h-[65%] h-full overflow-hidden mix-blend-multiply hover:scale-105 transition-all duration-200'>
        <img className='w-28 md:w-20 lg:w-36 mix-blend-multiply' src={`${productData.image}`} alt="" />
      </div>


      <div className='h-[2px] bg-neutral-700 w-full m-3'></div>
